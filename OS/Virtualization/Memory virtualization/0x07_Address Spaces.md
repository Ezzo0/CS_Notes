...
- When, for example, process A in Figure 13.3 tries to perform a load at address 0 (which we will call a virtual address), somehow the OS, in tandem with some hardware support, will have to make sure the load doesn’t actually go to physical address 0 but rather to for example physical address 320KB (where A is loaded into memory).
##### Goals
...
- In implementing time-efficient virtualization, the OS will have to rely on hardware support, including hardware features such as [[0x12_Faster Translations (TLBs)|TLBs]].
- Finally, a third VM goal is protection. The OS should make sure to protect [[0x01_Process|processes]] from one another as well as the OS itself from processes.
