...
- Unfortunately, the approach we have developed thus far contains serious flaws.
	...
	- The algorithm we have described is susceptible to the following attack: 
		- Before the allotment is used, issue an I/O operation (e.g., to a file) and thus relinquish the CPU; doing so allows you to remain in the same queue, and thus gain a higher percentage of CPU time. 
